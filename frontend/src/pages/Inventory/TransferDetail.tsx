...
import { transfersApi, inspectionApi, storesApi, productsApi } from '../../api';
    ...
    <div className="max-w-4xl mx-auto space-y-5">
        ...
        <div className="flex items-start justify-between flex-wrap gap-4 mb-5">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {canManage && transfer.status === 'approved' && (
              <button onClick={() => action('dispatched', () => transfersApi.dispatch(Number(id)))} disabled={!!acting}
                className="px-4 py-2 text-sm bg-orange-500 text-white rounded-lg hover:bg-orange-600 disabled:opacity-50">
                {acting === 'dispatched' ? 'Dispatching...' : 'Dispatch'}
              </button>
            )}
            {canManage && transfer.status === 'dispatched' && transfer.requires_inspection && (
              <button onClick={() => navigate(`/inspection/requests/new?source_type=TRANSFER&source_id=${id}&dst_store_id=${transfer.destination_store_id}&dst_store_name=${encodeURIComponent(transfer.destination_store_name)}`)}
                className="px-4 py-2 text-sm bg-yellow-500 text-white rounded-lg hover:bg-yellow-600">
                Create Inspection
              </button>
            )}
            {canManage && transfer.status === 'dispatched' && !transfer.requires_inspection && (
              <button onClick={() => action('received', () => transfersApi.receive(Number(id)))} disabled={!!acting}
            ...
            {!['cancelled','closed','received'].includes(transfer.status) && canManage && (
              <button onClick={() => { if (confirm('Cancel this transfer?')) action('cancelled', () => transfersApi.cancel(Number(id))); }}
                disabled={!!acting}
          ...
          {STATUS_STEPS.map((step, i) => {
            ...
            return (
              <div key={step} className="flex items-center flex-shrink-0">
                <div className={`flex flex-col items-center ${i < STATUS_STEPS.length - 1 ? 'min-w-[100px]' : ''}`}>
                  ...
                  <span className={`text-[9px] mt-1 text-center capitalize ${current ? 'text-brand-600 dark:text-brand-400 font-semibold' : 'text-gray-400'}`}>
                    {step.replace(/_/g,' ')}
                  </span>
                </div>
                {i < STATUS_STEPS.length - 1 && (
                  <div className={`h-0.5 flex-1 min-w-[20px] mx-1 mb-4 ${done ? 'bg-green-400' : 'bg-gray-200 dark:bg-gray-700'}`}/>
                )}
              </div>
            );
          })}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-brand-200 dark:border-brand-500/30 bg-brand-50/20 dark:bg-brand-500/5 p-5">
          ...
          {transfer.received_at && <p className="text-xs text-gray-400 mt-1">Received: {fmtDate(transfer.received_at)}</p>}
        </div>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="text-xs text-gray-400 border-b border-gray-100 dark:border-gray-800">
                {['Product','SKU','Unit','Transfer Qty','Current Stock'].map(h =>
      ...
      {transfer.requires_transit && (
        <Section title="Transit / Logistics">
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
            {[
              { label: 'Method',          value: transfer.transit_method ?? '—' },
              { label: 'Provider',        value: transfer.transit_provider ?? '—' },
              { label: 'Tracking #',      value: transfer.tracking_number ?? '—' },
              { label: 'Expected Arrival',value: fmtDate(transfer.expected_arrival_date) },
              { label: 'Vehicle',         value: transfer.vehicle_information ?? '—' },
              { label: 'Driver',          value: transfer.driver_information ?? '—' },
            ].map(item => (
              <div key={item.label}>
